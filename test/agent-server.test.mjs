...
const {
    ...
    MODES,
    MODE_ALIASES,
} = await import('../runtime/mcp/agent-server.mjs');
...
async function main() {
    ...
    assert.strictEqual(env.VANNEVAR_AGENT_DEPTH, '1', 'depth is stamped for the child');
    ...
    assert.strictEqual(env.VANNEVAR_AGENT_DEPTH, '2', 'depth increments from the caller');
    ...
    process.env.VANNEVAR_CLAUDE_BIN = path.join(dir, 'no-such-claude-binary');
    ...
    delete process.env.VANNEVAR_CLAUDE_BIN;

    // --- the depth guard stops an agent chain from spawning itself forever
    process.env.VANNEVAR_AGENT_DEPTH = '2';
    out = await capture(() =>
    ...
    delete process.env.VANNEVAR_AGENT_DEPTH;
