...
const HOST_REQUIRE =
    '/*__ccx*/let __p=require("path").join(require("os").homedir(),".claude","vannevar","host.js");' +
...
// The one line the automatic callers parse. `--if-needed` is what extension.js and the in-session
// watcher run, and both have to tell "nothing to do" from "the patch was just re-applied, this window
// is running the old bundle" — the second one is what earns a reload prompt.
const RESULT = {
...
// What the extension stamped beside this copy of the patcher: its own version, the Claude Code releases
// the signatures were checked against, and the bundle the extension host resolved through the VS Code
// API. The extension's own package.json is not above this file — the runtime lives in ~/.claude/vannevar
// — so the stamp is the only place these can come from.
//
// It is also where the extensions root comes from. A hand-run `apply-patch.mjs` with no --dir has to
// guess, and guessing ~/.vscode/extensions is wrong in Cursor, Windsurf, Insiders and on a Remote-SSH
// host; the parent of the path the extension resolved is right in all of them.
function stampMeta() {
    try {
        const json = JSON.parse(readFileSync(new URL('./patch-version.json', import.meta.url), 'utf8'));
        return {
            version: json.version || null,
            verifiedAgainst: Array.isArray(json.verifiedAgainst) ? json.verifiedAgainst : [],
            extensionPath: typeof json.extensionPath === 'string' ? json.extensionPath : null,
        };
    } catch {
        return { version: null, verifiedAgainst: [], extensionPath: null };
    }
}

const STAMP = stampMeta();

function extensionsRoot() {
    if (STAMP.extensionPath) return path.dirname(STAMP.extensionPath);
...
// The extension carries its own version now, so "supported" is no longer one number that has to equal
// the installed release — it is the list of releases somebody actually looked at. A release not on that
// list is unverified, which is a warning and never a stop: most signatures match the shape of the code
// rather than the names in it, so a new release usually takes the patch fine.
function checkVersion(dir) {
    const verified = STAMP.verifiedAgainst;
    const installed = (path.basename(dir).match(/anthropic\.claude-code-(\d+\.\d+\.\d+)/) || [])[1];
    const differs = Boolean(verified.length && installed && !verified.includes(installed));
    const supported = verified.join(', ') || 'unknown';
    console.log(`patcher:   ${STAMP.version || 'unknown'} (verified against ${supported})`);
    if (differs) {
        console.log(`  installed: ${installed}`);
        console.log('  WARNING: this release was not verified. If the signatures changed, the patch stops with an error.');
    }
    return { supported: verified[0] || null, installed, differs };
}
...
// process.execPath is Code.exe when this runs from inside the extension host (extension.js and
...
// callers can genuinely collide: VS Code restores every window at once, each one activates this extension,
// and each activation finds the same unpatched bundle. So the decision and the write happen together under
...
if (process.argv.includes('--revert')) restore(dir);
    ...
    // Nothing is fetched and nothing heals itself: a patch that no longer fits is reported, and the
    // fix arrives the way every other extension update does — through the VS Code updater, which
    // installs a Vannevar Code that carries signatures checked against this release.
