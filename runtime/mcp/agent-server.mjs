...
// one place a provider can still change is where a process is born, which is what Vannevar already
...
const PROFILES_DIR = process.env.VANNEVAR_PROFILES_DIR || path.join(HOME, '.claude', 'profiles');
const RUNTIME = process.env.VANNEVAR_RUNTIME_DIR || path.join(HOME, '.claude', 'vannevar');
...
const DEPTH_VAR = 'VANNEVAR_AGENT_DEPTH';
const MAX_DEPTH = 2;
...
const PARENT_VAR = 'VANNEVAR_PARENT_RUN';
...
// Optional, and only Vannevar reads it — the CLI never sees this key. USD per million tokens,
...
async function preflight(env, requestedModel, profile) {
    if (process.env.VANNEVAR_SKIP_PREFLIGHT) return null;
...
const EXEC_WRAPPERS = (process.env.VANNEVAR_EXEC_WRAPPERS ?? 'rtk')
    .split(',')
    .map((s) => s.trim())
    .filter(Boolean);
